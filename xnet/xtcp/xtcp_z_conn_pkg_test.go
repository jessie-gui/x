...
func TestPackageBasic(t *testing.T) {
	...
	// SendPkg
	conn1, err1 := xtcp.NewConn(s.GetListenedAddress())
	assert.Nil(t, err1)
	defer conn1.Close()
	for i := 0; i < 100; i++ {
	...
	conn2, err2 := xtcp.NewConn(s.GetListenedAddress())
	assert.Nil(t, err2)
	defer conn2.Close()
	...
	// SendReceivePkg
	conn3, err3 := xtcp.NewConn(s.GetListenedAddress())
	assert.Nil(t, err3)
	defer conn3.Close()
	for i := 100; i < 200; i++ {
	...
	conn4, err4 := xtcp.NewConn(s.GetListenedAddress())
	assert.Nil(t, err4)
	defer conn4.Close()
	...
	conn5, err5 := xtcp.NewConn(s.GetListenedAddress())
	assert.Nil(t, err5)
	defer conn5.Close()
...
func TestPackageBasicHeaderSize1(t *testing.T) {
	...
	s := xtcp.NewServer(xtcp.FreePortAddress, func(conn *xtcp.Conn) {
		defer conn.Close()
		for {
			data, err := conn.ReceivePkg(xtcp.PkgOption{HeaderSize: 1})
			if err != nil {
				break
			}
			conn.SendPkg(data)
...
func TestPackageTimeout(t *testing.T) {
	...
	defer conn3.Close()
	data3 := []byte("10000")
	result3, err3 := conn3.SendReceivePkgWithTimeout(data3, time.Second*2, xtcp.PkgOption{HeaderSize: 5})
...
func TestPackageOption(t *testing.T) {
	...
	s := xtcp.NewServer(xtcp.FreePortAddress, func(conn *xtcp.Conn) {
		defer conn.Close()
		option := xtcp.PkgOption{HeaderSize: 1}
		for {
	...
	result1, err1 := conn1.SendReceivePkg(data1, xtcp.PkgOption{HeaderSize: 1})
	assert.NotNil(t, err1)
	assert.Nil(t, result1)
	...
	data2[100] = byte(65)
	data2[200] = byte(85)
	result2, err2 := conn2.SendReceivePkg(data2, xtcp.PkgOption{HeaderSize: 1})
	assert.Nil(t, err2)
	assert.Equal(t, result2, data2)
}
...
func TestPackageOptionHeadSize3(t *testing.T) {
	...
	s := xtcp.NewServer(xtcp.FreePortAddress, func(conn *xtcp.Conn) {
		defer conn.Close()
		option := xtcp.PkgOption{HeaderSize: 3}
		for {
			data, err := conn.ReceivePkg(option)
			if err != nil {
				break
	...
	data[100] = byte(65)
	data[200] = byte(85)
	result, err := conn.SendReceivePkg(data, xtcp.PkgOption{HeaderSize: 3})
	assert.Nil(t, err)
...
func Test_Package_Option_HeadSize4(t *testing.T) {
	...
	s := xtcp.NewServer(xtcp.FreePortAddress, func(conn *xtcp.Conn) {
		defer conn.Close()
		option := xtcp.PkgOption{HeaderSize: 4}
		for {
			data, err := conn.ReceivePkg(option)
			if err != nil {
				break
	...
	_, err1 = conn1.SendReceivePkg(data1, xtcp.PkgOption{HeaderSize: 4})
	assert.Nil(t, err1)
	...
	data2[100] = byte(65)
	data2[200] = byte(85)
	result2, err2 := conn2.SendReceivePkg(data2, xtcp.PkgOption{HeaderSize: 4})
	assert.Nil(t, err2)
	assert.Equal(t, result2, data2)
	...
	data3[100] = byte(65)
	data3[200] = byte(85)
	_, err3 = conn3.SendReceivePkg(data3, xtcp.PkgOption{HeaderSize: 5})
	assert.NotNil(t, err3)
}
...
func TestConnReceivePkgError(t *testing.T) {
	...
	s := xtcp.NewServer(xtcp.FreePortAddress, func(conn *xtcp.Conn) {
		defer conn.Close()
		option := xtcp.PkgOption{HeaderSize: 5}
		for {
