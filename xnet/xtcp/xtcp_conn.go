	...
	for {
		if _, err := c.Write(data); err != nil {
			...
			if len(retry) == 0 || retry[0].Count == 0 {
				err = xerror.Wrap(err, `Write data failed`)
				return err
			}
			if len(retry) > 0 {
				retry[0].Count--
	...
	for {
		if length < 0 && index > 0 {
			...
			if err = c.SetReadDeadline(time.Now().Add(c.receiveBufferWait)); err != nil {
				err = xerror.Wrap(err, `SetReadDeadline for connection failed`)
				return nil, err
			}
		}
...
func (c *Conn) SetDeadline(t time.Time) (err error) {
	...
	if err != nil {
		err = xerror.Wrapf(err, `SetDeadline for connection failed with "%s"`, t)
	}
	return err
}
...
func (c *Conn) SetReceiveDeadline(t time.Time) (err error) {
	...
	if err != nil {
		err = xerror.Wrapf(err, `SetReadDeadline for connection failed with "%s"`, t)
	}
	return err
}
...
func (c *Conn) SetSendDeadline(t time.Time) (err error) {
	...
	if err != nil {
		err = xerror.Wrapf(err, `SetWriteDeadline for connection failed with "%s"`, t)
	}
	return err
}
