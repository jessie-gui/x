...
func (c *Conn) SendPkg(data []byte, option ...PkgOption) error {
	...
	if length > pkgOption.MaxDataSize {
		return xerror.NewCodef(
			xcode.CodeInvalidParameter,
...
func (c *Conn) ReceivePkg(option ...PkgOption) (result []byte, err error) {
	...
	if length < 0 || length > pkgOption.MaxDataSize {
		return nil, xerror.NewCodef(xcode.CodeInvalidParameter, `invalid package size %d`, length)
	}
	// 空包。
	if length == 0 {
		return nil, nil
