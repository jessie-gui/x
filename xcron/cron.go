...
func New(opts ...Option) *Cron {
	c := &Cron{
		...
		running:   false,
		runningMu: sync.Mutex{},
		logger:    xlog.NewHelper(xlog.GetLogger()),
		location:  time.Local,
...
func (c *Cron) run() {
	...
	for _, entry := range c.entries {
		...
		// c.logger.Infow(xlog.DefaultMessageKey, "Cron", "action", "schedule", "now", now, "entry", entry.ID, "next", entry.Next)
	}
	...
	for {
		...
		for {
			...
			case now = <-timer.C:
				...
				// c.logger.Infow(xlog.DefaultMessageKey, "Cron", "action", "wake", "now", now)
				...
				for _, e := range c.entries {
					...
					// c.logger.Infow(xlog.DefaultMessageKey, "Cron", "action", "run", "now", now, "entry", e.ID, "next", e.Next)
				}
			...
			case newEntry := <-c.add:
				...
				// c.logger.Infow(xlog.DefaultMessageKey, "Cron", "action", "added", "now", now, "entry", newEntry.ID, "next", newEntry.Next)
			...
			case id := <-c.remove:
				...
				now = c.now()
				c.removeEntry(id)
				// c.logger.Infow(xlog.DefaultMessageKey, "Cron", "action", "removed", "entry", id)
			}
