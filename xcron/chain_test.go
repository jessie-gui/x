...
func TestChainRecover(t *testing.T) {
	...
	t.Run("Recovering JobWrapper recovers", func(t *testing.T) {
		NewChain(Recover(xlog.NewHelper(xlog.GetLogger()))).
			Then(panickingJob).
			Run()
	})
...
func TestChainDelayIfStillRunning(t *testing.T) {
	...
	t.Run("runs immediately", func(t *testing.T) {
		var j countJob
		wrappedJob := NewChain(DelayIfStillRunning(xlog.NewHelper(xlog.GetLogger()))).Then(&j)
		go wrappedJob.Run()
...
func TestChainSkipIfStillRunning(t *testing.T) {
	...
	t.Run("runs immediately", func(t *testing.T) {
		var j countJob
		wrappedJob := NewChain(SkipIfStillRunning(xlog.NewHelper(xlog.GetLogger()))).Then(&j)
		go wrappedJob.Run()
		time.Sleep(2 * time.Millisecond) // Give the job 2ms to complete.
		if c := j.Done(); c != 1 {
			t.Errorf("expected job run once, immediately, got %d", c)
	...
	t.Run("different jobs independent", func(t *testing.T) {
		...
		chain := NewChain(SkipIfStillRunning(xlog.NewHelper(xlog.GetLogger())))
